...
if(isset($_POST['submit'])){
    ...
    if($email=="" || $uname=="" || $password=="" || $re_password==""){
        ...
        if($password==$re_password){
            $query=mysqli_query($connection,"select * from buyer_login where Uname='$uname' or Email='$email'");
            if($query){
                if(mysqli_num_rows($query)>0){
                    ...
                    $query=mysqli_query($connection,"insert into buyer_login (Uname,Password,Email)values('$uname','$password','$email')");
                    if($query){
